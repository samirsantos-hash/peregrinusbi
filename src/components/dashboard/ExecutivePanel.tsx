...
const ExecutivePanel = ({ kpis }: ExecutivePanelProps) => {
  ...
  // Saúde da Operação (score_final_full avg)
  ...
  const metrics = [
    {
      ...
      icon: DollarSign,
      color: "neon-text",
      tooltip: "Valor total das vendas brutas no período selecionado (GMV = Gross Merchandise Value).",
    },
    {
      label: "Faturamento Realizado",
      value: `R$ ${(totalTgmv / 1000).toFixed(0)}K`,
      icon: TrendingUp,
      color: "emerald-text",
      tooltip: "Valor de vendas confirmadas e faturadas, descontando cancelamentos e devoluções.",
    },
    {
      label: "Volume de Itens Vendidos",
      value: totalTsi.toLocaleString("pt-BR"),
      icon: ShoppingCart,
      color: "neon-text",
      tooltip: "Quantidade total de itens vendidos (TSI = Total Sold Items) no período.",
    },
    {
      label: "Investimento em Marketing",
      value: `R$ ${(totalAds / 1000).toFixed(0)}K`,
      icon: Activity,
      color: "text-muted-foreground",
      tooltip: "Total investido em campanhas de Ads (Product Ads) no Mercado Livre.",
    },
    {
      label: "Potencial de Crescimento",
      value: `${avgUplift >= 0 ? "+" : ""}${(avgUplift * 100).toFixed(1)}%`,
      icon: avgUplift >= 0 ? TrendingUp : TrendingDown,
      color: avgUplift >= 0 ? "emerald-text" : "text-destructive",
      tooltip: "Calculado comparando sua performance real com a média esperada da sua Vertical e Domínio.",
    },
    {
      label: "Nota de Saúde da Operação",
      value: avgSaude > 0 ? avgSaude.toFixed(0) : "—",
      icon: Activity,
      color: avgSaude >= 70 ? "emerald-text" : avgSaude >= 50 ? "text-warning" : "text-destructive",
      tooltip: "Média ponderada de preço, qualidade de fotos, descrições e logística.",
    },
    {
      label: "ROAS Médio",
      value: avgRoas.toFixed(2),
      icon: DollarSign,
      color: avgRoas >= 2 ? "emerald-text" : "text-destructive",
      tooltip: "Retorno sobre investimento em Ads. Acima de 2x é considerado saudável.",
    },
    {
      label: "Índice de Atrasos no Envio",
      value: avgDelayed > 0 ? `${(avgDelayed * 100).toFixed(1)}%` : "—",
      icon: Clock,
      color: avgDelayed <= 0.05 ? "emerald-text" : "text-destructive",
      tooltip: "Percentual de envios atrasados. Abaixo de 5% é considerado saudável para manter reputação.",
    },
  ];
