...
const QualityPanel = ({ kpis }: QualityPanelProps) => {
  ...
  const avgPhoto = Math.round(products.reduce((s, p) => s + p.scorePhoto, 0) / products.length);
  const avgTitle = Math.round(products.reduce((s, p) => s + p.scoreTitle, 0) / products.length);
